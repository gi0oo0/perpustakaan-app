                ...
                <div class="lg:col-span-2">
                    <div class="neo-card">
                        ...
                        <div class="mt-6 border-t-3 border-border pt-6">
                            ...
                            <form id="return-form" method="POST" action="{{ route('loans.return.store') }}" class="flex gap-2">
                                @csrf
                                <input type="text" id="isbn" name="isbn"
                                       class="neo-input flex-1"
                                       placeholder="Masukkan ISBN..."
                                       autocomplete="off">
                                <button type="submit" class="neo-btn-primary whitespace-nowrap">→ Kembalikan</button>
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        @else
                            <div class="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                                @foreach ($activeLoans as $loan)
                                    <div class="border-2 border-border p-3 {{ $loan->due_date < \Carbon\Carbon::today() ? 'bg-red-50 border-coral' : '' }} transition-colors duration-100" style="box-shadow: 3px 3px 0px #111827;">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="font-body text-xs text-muted">ISBN: {{ $loan->book->isbn }}</p>
                                                <p class="font-body text-xs {{ $loan->due_date < \Carbon\Carbon::today() ? 'text-coral font-semibold' : 'text-muted' }}">
                                        ...
                                        <form method="POST" action="{{ route('loans.return.store') }}" onsubmit="return confirm('Yakin ingin mengembalikan buku ini?')" class="mt-2">
                                            @csrf
                                            <input type="hidden" name="loan_id" value="{{ $loan->id }}">
                                            <button type="submit" class="neo-btn-danger w-full text-xs py-1">Kembalikan</button>
                                        </form>
                                    </div>
                                @endforeach
                            </div>
