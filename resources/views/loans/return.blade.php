            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <p class="text-sm text-gray-500 mb-4">Klik tombol di bawah untuk mengaktifkan kamera, lalu arahkan ke barcode buku yang akan dikembalikan.</p>
                        ...
                        <form id="return-form" method="POST" action="{{ route('loans.return.store') }}" class="mt-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="isbn" name="isbn" type="text" class="mt-1 block w-full text-lg"
                                    placeholder="ISBN terisi otomatis dari scan..." autocomplete="off" />
                                <x-input-error :messages="$errors->get('isbn')" class="mt-2" />
                            </div>
                            <div class="mt-4">
                                <x-primary-button type="submit">{{ __('Kembalikan') }}</x-primary-button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold mb-4">Buku Yang Sedang Dipinjam</h3>
                        @if ($activeLoans->isEmpty())
                            <p class="text-gray-500 text-sm">Tidak ada buku yang sedang dipinjam.</p>
                        @else
                                    ...
                                    <div class="border rounded-lg p-3 flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            @if ($loan->book->cover_image)
                                                <img src="{{ asset($loan->book->cover_image) }}" alt="{{ $loan->book->title }}" class="h-12 w-9 object-cover rounded">
                                            @else
                                                <div class="h-12 w-9 bg-gray-200 rounded flex items-center justify-center text-gray-400 text-xs">N/A</div>
                                            @endif
                                            <div class="min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate">{{ $loan->book->title }}</p>
                                                <p class="text-xs text-gray-500">ISBN: {{ $loan->book->isbn }}</p>
                                                <p class="text-xs {{ $loan->due_date < \Carbon\Carbon::today() ? 'text-red-600 font-semibold' : 'text-gray-500' }}">
                                                    Jatuh tempo: {{ $loan->due_date->format('d/m/Y') }}
                                                    @if ($loan->due_date < \Carbon\Carbon::today())
                                                        (Terlambat!)
                                                    @endif
                                                </p>
                                            </div>
                                        </div>
                                        <form method="POST" action="{{ route('loans.return.store') }}" onsubmit="return confirm('Yakin ingin mengembalikan buku ini?')">
                                            @csrf
                                            <input type="hidden" name="loan_id" value="{{ $loan->id }}">
                                            <x-danger-button class="text-xs">{{ __('Kembalikan') }}</x-danger-button>
                                        </form>
                                    </div>
                                @endforeach
                            </div>
            ...
            function startCamera(config) {
                ...
                return html5QrCode.start(
                    ...
                    function onScanSuccess(decodedText) {
                        ...
                        setTimeout(function () { returnForm.submit(); }, 500);
                    },
                    function () {}
                );
            }
